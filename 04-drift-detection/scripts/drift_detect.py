...
def main() -> int:
    ...
    # Optional: full Evidently HTML report
    # Supports both evidently 0.4.x (save_html) and 0.7.x (different API)
    try:
        ...
        # evidently 0.7.x: use _save() method
        if hasattr(report, "save_html"):
            report.save_html(str(html_path))
        elif hasattr(report, "_save"):
            report._save(str(html_path))
        else:
            # Fallback: write a simple HTML summary from the drift data
            rows = "".join(
                f"<tr><td>{col}</td><td>{m['psi']}</td><td>{m['kl']}</td>"
                f"<td>{m['ks_stat']}</td><td>{m['ks_pvalue']}</td>"
                f"<td style='color:{'red' if m['drift']=='yes' else 'green'}'>{m['drift']}</td></tr>"
                for col, m in summary.items()
            )
            html_path.write_text(
                "<html><head><title>Day 23 — Drift Report</title>"
                "<style>body{font-family:monospace;margin:2em} table{border-collapse:collapse}"
                "th,td{border:1px solid #ccc;padding:8px 16px;text-align:right}"
                "th{background:#f5f5f5}</style></head><body>"
                "<h1>Drift Detection Report</h1>"
                "<table><tr><th>Feature</th><th>PSI</th><th>KL</th><th>KS</th><th>KS p-value</th><th>Drift</th></tr>"
                f"{rows}</table>"
                "<p><em>Generated by 04-drift-detection/scripts/drift_detect.py</em></p>"
                "</body></html>"
            )
        print(f"Wrote: {html_path}")
    except ImportError as e:
        print(f"evidently not installed; skipping HTML report ({e}). Install with: pip install evidently")
    except Exception as e:
        print(f"evidently report generation failed: {e}")
        # Write fallback HTML anyway
        rows = "".join(
            f"<tr><td>{col}</td><td>{m['psi']}</td><td>{m['kl']}</td>"
            f"<td>{m['ks_stat']}</td><td>{m['ks_pvalue']}</td>"
            f"<td style='color:{'red' if m['drift']=='yes' else 'green'}'>{m['drift']}</td></tr>"
        ...
        html_path.write_text(
            ...
            "<table><tr><th>Feature</th><th>PSI</th><th>KL</th><th>KS</th><th>KS p-value</th><th>Drift</th></tr>"
            f"{rows}</table>"
            "<p><em>Generated by 04-drift-detection/scripts/drift_detect.py (fallback HTML)</em></p>"
            "</body></html>"
        )
